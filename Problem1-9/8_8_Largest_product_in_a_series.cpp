#include <iostream>
#include <stack>
#include <string>
int main()
{
    int seq_degit = 13;
    std::string num = ("");
    int degit = (int)num.size();
    std::cout << degit << "桁の数字について"
        << seq_degit << "個の連続した整数積を求めています。" << std::endl;
    std::stack<unsigned long long int> results;
    results.push(0);
    bool start = true;
    unsigned long long int result = 1;
    int i = 0;
    while (i < degit - seq_degit - 1) {
        std::cout << i << std::endl;
        if (start == true) {
            std::cout << "start";
            unsigned long long int hoge = 1;
            for (int p = 0; p < seq_degit; p++) {
                hoge *= std::stoi(std::string(1, num.at(i + p)));
            }
            std::cout << hoge << "hoge" << std::endl;
            if (hoge != 0) {
                if (result < hoge) {
                    results.push(hoge);
                    std::cout << results.top() << "ほげー" << std::endl;
                }
                if (std::stoi(std::string(1, num.at(i + seq_degit - 1))) != 0) {
                    start = false;
                }
                if (std::stoi(std::string(1, num.at(i + seq_degit))) != 0) {
                    i++;
                }
                result = hoge;
            } else {
                i += 1;
            }
        } else {
            std::cout << "started";
            if (std::stoi(std::string(1, num.at(i + seq_degit))) == 0) {
                unsigned long long int hoge = 1;
                for (int p = 0; p < seq_degit; p++) {
                    hoge *= std::stoi(std::string(1, num.at(i + p)));
                }
                if (hoge > result) {
                    results.push(hoge);
                    result = hoge;
                    std::cout << results.top() << "マジほげ" << std::endl;
                }
                i += seq_degit;
                result = 1;
                start = true;
            } else if (std::stoi(std::string(1, num.at(i + seq_degit))) > std::stoi(std::string(1, num.at(i)))) {
                int hoge = 1;
                for (int p = 0; p < seq_degit; p++) {
                    hoge *= std::stoi(std::string(1, num.at(i + p)));
                }
                results.push(result);
                std::cout << std::stoi(std::string(1, num.at(i))) << std::stoi(std::string(1, num.at(i + seq_degit))) << std::endl;
                std::cout << results.top() << "ほげじゃん" << std::endl;
                i++;
            } else {
                i++;
            }
        }
    }
    int size = (int)results.size();
    std::cout << size << "size" << std::endl;
    for (int p = 0; p < size; p++) {
        if (results.top() > result) {
            result = results.top();
        }
        results.pop();
    }
    std::cout << "The result is " << result << "." << std::endl;
    return 0;
}
